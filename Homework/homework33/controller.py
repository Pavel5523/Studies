    ...
    def check_user_answer(self, answer):
        if answer == '1':
            ...
            try:
                ...
                self.user_interface.show_inkorrect_name_error(film_name)
        elif answer == '4':
            film_name = self.user_interface.get_user_film()
            try:
                name = self.film_model.remove_film(film_name)
                self.user_interface.remove_singl_film(name)
            except KeyError:
                self.user_interface.show_inkorrect_name_error(film_name)
        elif answer == 'q':
            self.film_model.save_data()
        else:
            self.user_interface.show_inkorrect_answer_error(answer)
